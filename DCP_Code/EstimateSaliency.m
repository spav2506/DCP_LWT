...
%             delete(OUT_VDO);
            ...
            for frame = 1:FRMS_CNT
                if frameType(frame) == 'P'
                    [mv_x(:,:,frame), mv_y(:,:,frame)] = ReadMVs(IN_MV, frame, BLK_H, BLK_W, 0.5);
                end
            end
            for frame = 1:FRMS_CNT
...
% % % % % % % % % % % %             times = toc/num;
% % % % % % % % % % % %             disp(times*1000)
% % % % % % % % % % % %             resultname = [SEQ_DIR SEQ_NAME filesep 'result_PROPOSED(LDP_LWT)_' FORMAT '.mat'];
% % % % % % % % % % % %             save(resultname,'S','times')
%             resultname = [SEQ_DIR SEQ_NAME filesep 'result_DWT_' FORMAT '.mat'];
%             save(resultname,'S','times')


%             disp('PROPOSED(ldp2)')
%             tic
%             [S]= method_LDP2(frameType,mv_x,mv_y,mbDct);
%             num = sum(frameType=='P');
%             times = toc/num;
%             disp(times*1000)
%             resultname = [SEQ_DIR SEQ_NAME filesep 'result_Krondelta_' FORMAT '.mat'];
%             save(resultname,'S','times')  
%             
%             disp('PROPOSED(ldp)')
%             tic
...
%             times = toc/num;
%             disp(times*1000)
%             resultname = [SEQ_DIR SEQ_NAME filesep 'result_Addition_' FORMAT '.mat'];
%             save(resultname,'S','times')
%             
%             disp('PROPOSED(ldp1)')
%             tic
%             [S]= method_LDP1(frameType,mv_x,mv_y,mbDct);
%             num = sum(frameType=='P');
%             times = toc/num;
%             disp(times*1000)
%             resultname = [SEQ_DIR SEQ_NAME filesep 'result_Multiplication_' FORMAT '.mat'];
%             save(resultname,'S','times')        
